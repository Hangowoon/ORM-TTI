...
router.get('/', async (req, res, next) => {
  res.render('tti/login.ejs', {
    loginResult: '',
  });
});

// express-session 기반으로 로그인 구현
router.post('/', async (req, res, next) => {
  ...
  // 2. 데이터 체크
  // 1) 동일한 메일 존재하는지 체크
  var user = await db.User_data.findOne({
  ...
  if (user == null) {
    ...
    var isCorrentPwd = await bcrypt.compare(userPassword, user.user_password);
    ...
    if (isCorrentPwd) {
      ...
      req.session.loginUser = {
        ...
        user_profile_img_path: user.user_profile_img_path,
